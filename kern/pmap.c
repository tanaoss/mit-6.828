...
i386_detect_memory(void)
{
	...
	//cprintf("npage:%u, npages_basemem:%u\n",npages,npages_basemem);
}
...
mem_init(void)
{
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
	boot_map_region(kern_pgdir, KERNBASE, ~(uint32_t)0 + 1- KERNBASE , 0, PTE_W);
...
page_init(void)
{
	...
	//distribution the memory that before IOPHYSMEM
	pages[1].pp_ref = 0;
	pages[1].pp_link=NULL;
	page_free_list=&pages[1];
	for (i = 2; i < npages_before_mp; i++) {
		pages[i].pp_ref = 0;
    ...
    size_t mpentry_npages = (mpentry_len+PGSIZE-1) / PGSIZE;
	...
	//distribution the memory that after EXTPHYSMEM
...
page_alloc(int alloc_flags)
{
	...
	target->pp_link=NULL;
	target->pp_ref=0;
	char* space_head=page2kva(target);
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	for(;va-start_va<size;va+=PGSIZE,pa+=PGSIZE){
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	if (PTE_ADDR(*pte) != page2pa(pp)) {//only in this is the first time acllocation ,we add it!
	    ++pp->pp_ref;
	}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	perm |= PTE_P | PTE_U;
	const void* va_cbegin=ROUNDDOWN(va,PGSIZE);
	const void* va_cend=ROUNDUP(va+len,PGSIZE);
	pte_t* pte=pgdir_walk(env->env_pgdir,va_cbegin,0);
	if(pte==NULL||((*pte&perm )!=perm)){
			user_mem_check_addr= (uintptr_t)va;
			return -E_FAULT;
	}
	for(va=va_cbegin+PGSIZE;va<va_cend;va+=PGSIZE){
